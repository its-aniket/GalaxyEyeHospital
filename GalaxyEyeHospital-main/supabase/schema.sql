...
-- ─── Eye Care at Home Leads ───
CREATE TABLE home_consultation_leads (
  id SERIAL PRIMARY KEY,
...
-- ─── Eye Photo Storage Bucket ───
-- image_url stores the storage object path for a private bucket.
...
ALTER TABLE home_consultation_leads ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "public_insert_home_consultation_leads" ON home_consultation_leads FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "staff_read_home_consultation_leads" ON home_consultation_leads FOR SELECT TO authenticated USING (true);
CREATE POLICY "public_upload_eye_photos" ON storage.objects FOR INSERT TO anon WITH CHECK (bucket_id = 'eye-photos');
CREATE POLICY "staff_read_eye_photos" ON storage.objects FOR SELECT TO authenticated USING (bucket_id = 'eye-photos');
...
CREATE INDEX idx_home_consultation_leads_status ON home_consultation_leads (status);
CREATE INDEX idx_home_consultation_leads_created_at ON home_consultation_leads (created_at DESC);
